...
extract.rules <- function (data = data.split$train.data, n.trees = 100, mtry = 5, node_depth_max = 7) {
  ...
  for (j in 1:total_num_rules) {
    ...
    # the rule j does not have an 'else':
  ...
  final_result_temp_1 <- final_result[!is.na(final_result[,3]),]
  
  # remove rules with performance scores = 0
  final_result_temp_2 <- final_result_temp_1[final_result_temp_1[,3] != 0,]
  ...
  # comes from using %>% in the above step:
  final_result_all <- final_result_all[,-6]
  
  colnames(final_result_all) <- c("Rules", "Class Label", "Performance Score",
  ...
  for (i in 1:nrow(rules.order)) {
    ...
    if (i > nrow(rules.order)) {
      ...
      for (k in 1:dim(validation.data)[1]) {
        ...
        label <- table(as.character(complete.cases(inter.table[,1])))
      ...
      remove <- which(pred_class[,1] == "UNS")
      
      if (length(remove) == 0) {
        comp_table <- comp_table
      } else if (length(remove) != 0) {
        comp_table <- comp_table[-remove,]
      }
        ...
        for (w in 1:nrow(decision.rules))  {
          if (eval(parse(text = decision.rules[w,]$Rules)) == T) {
            inter.table[w,] <- list("label" = decision.rules[w,][,2], 
                                    "Performance Score" = decision.rules[w,][,3], 
                                    "Indicator" = 1)
            # indicator = 1: the sample's label is determined by 'class label'
