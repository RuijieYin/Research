...
# extract rules:
rules <- extract.rules(data = data.split$train.data, n.trees = 300, mtry = 5, node_depth_max = 7)
# see all rules extracted from the n.tree trees
write.csv(rules$Rules, "F:/Dropbox/UM Biostatistics/Research/1st Project/TNBCtype_research/Results/Decision Rules Results October 13/rules_extracted_exp1.csv")
...
write.csv(selected.rules$Selected.Rules, "F:/Dropbox/UM Biostatistics/Research/1st Project/TNBCtype_research/Results/Decision Rules Results October 13/multiclass1_rules_selected_exp1.csv")
