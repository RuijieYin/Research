...
# split training data:
split_one_vs_rest_data_train <- function(data = data.split$train.data) {
  num_classes <- length(unique(data$subtype))
  classes <- unique(data$subtype)
  train_data <- list()
  # for each class, create a one-vs-rest dataset, respectively
  for ( i in 1:num_classes ) {
    data_temp_1 <- data[data$subtype == classes[i],]
    data_temp_2 <- data[data$subtype != classes[i],]
    # change the class label to 'other'
    data_temp_2[,ncol(data_temp_2)] <- c(rep('other', nrow(data_temp_2)))
    ...
    assign(paste0("data.class.", i), rbind(data_temp_1,data_temp_2))
    train_data[[i]] <- get(paste0("data.class.", i))
  }
  return(train_data)
}

# training_data = split_one_vs_rest_data_train(data = data.split$train.data)


# split validation data:
split_one_vs_rest_data_validation <- function(data = data.split$validation.data) {
  num_classes <- length(unique(data$subtype))
  classes <- unique(data$subtype)
  ...
  for ( i in 1:num_classes ) {
    ...
    # change the class label to 'other'
    data_temp_2[,ncol(data_temp_2)] <- c(rep('other', nrow(data_temp_2)))
    # we have to refactorize the class labels in data_temp_1 and data_temp_2, to make sure only 2 classes are left
    data_temp_1[,ncol(data_temp_1)] <- factor(data_temp_1[,ncol(data_temp_1)])
    data_temp_2[,ncol(data_temp_2)] <- factor(data_temp_2[,ncol(data_temp_2)])
    assign(paste0("data.class.validation.", i), rbind(data_temp_1,data_temp_2))
    validation_data[[i]] <- get(paste0("data.class.validation.", i))
  }
  return(validation_data)
}

# validation_data = split_one_vs_rest_data_validation(data = data.split$validation.data)


# function 3: extract.rules
# model section
# balance class: when growing each tree for a one-vs-rest comparison in a forest, balance the sample of the 2 classes.
library(data.table)
...
extract.rules <- function (data = training_data, n.trees = 20, mtry = 5, node_depth_max = 5) {
  ...
  # set a initial seed number (can be any number)
  seed.num = 1
  
  # initialize tree.collection:
  tree.collection = list()
  
  # initialize a training data for each tree in a forest for each class:
  train.balanced <- list()
  
  # grow forests for a total of length(training_data) times, for each of the class:
  for (c in 1:length(data)) {
    # initialize an indicator for storing each obtained rules from trees:
    footstep = 1
    
    data_each_class = as.data.frame(data[[c]])
    for (h in 1:n.trees) {
      set.seed(seed.num)
      class.data = data_each_class[data_each_class$subtype != "other", ]
      other.data = data_each_class[data_each_class$subtype == "other", ]
      # some class may have larger samples than all other classes combined:
      if (dim(class.data)[1] <= dim(other.data)[1]) {
        train.balanced[[h]] <- rbind(class.data,
                                     other.data[sample(nrow(other.data), size = dim(class.data)[1], replace = FALSE), ])
      } else if (dim(class.data)[1] >= dim(other.data)[1]) {
        train.balanced[[h]] <- rbind(other.data,
                                     class.data[sample(nrow(class.data), size = dim(other.data)[1], replace = FALSE), ])
      }
      
      # growing trees for each class:
      tree.collection[[h]] <- rfsrc(subtype~., data = train.balanced[[h]],
                                    ntree = 2, mtry = mtry, nodedepth = node_depth_max,
                                    bootstrap = "by.root", samptype = 'swr',
                                    membership = T)
      # grow class balanced trees
      # case.wt = randomForestSRC:::make.wt(train.balanced$subtype),
      # sampsize = randomForestSRC:::make.size(train.balanced$subtype))
      seed.num = seed.num + 1
    }
    
    # calculate the total number of rules obtained from the above one-vs-rest forest:
    # total_num_rules <- length(getTreeRule(tree.collection)$tree.id)
    
    # calculate the total number of rules obtained from all trees of a class:
    # total_num_rules <- 0
    # for (i in 1:n.trees) {
    #   total_num_rules <- total_num_rules + length(getTreeRule(tree.collection[[i]])$treeRule)
    # }
    # 
    # initialize a data frame to store each rule (rules[[j]], class, else_class, as.numeric(perform.score))
    ...
    for (i in 1:n.trees) {
      # get the number of rules for a tree for a class:
      number.of.rules <- length(getTreeRule(tree.collection[[i]])$treeRule)
      # store the rules extracted from this tree for this class:
      ...
      for (j in 1:number.of.rules) {
        ...
        # tree.i <- getTreeRule(tree.collection[[i]])$tree.id[j]
        
        # index of inbag sample to grow tree i:
        # see notes 9.25.2022 on how to find inbag samples:
        index.table <- tree.collection[[i]]$inbag
        index.inbag <- which(index.table[,1] != 0)
        # find inbag samples suffice the jth rule:
        x <- train.balanced[[i]] # should tell what x is
        ...
        # determine the class label for this rule using the index sample:
        # should consider the tied class: 
        class_label_train <- table(train.balanced[[i]][inbag.sample.fit.rule, dim(train.balanced[[i]])[2]])
        ...
        inbag.sample.not.fit.rule <- index.inbag[!index.inbag %in% inbag.sample.fit.rule]
        # find the class labels of samples that does not suffice jth rule;
        majority_class_index <- which(as.character(train.balanced[[i]][inbag.sample.not.fit.rule, dim(train.balanced[[i]])[2]],class) %in% class)
        else_label_train <- table(as.character(train.balanced[[i]][inbag.sample.not.fit.rule, dim(train.balanced[[i]])[2]])[-majority_class_index])
        ...
        out.of.bag <- which(index.table[,1] == 0)
        oob.sample <- train.balanced[[i]][out.of.bag, ]
        # oob.sample.balanced <- oob.sample[complete.cases(oob.sample), ]
        oob.sample.balanced <- downSample(x = oob.sample[,-dim(oob.sample)[2]],
                                          y = factor(oob.sample[,dim(oob.sample)[2]]))
        ...
        if (is.na(else_class) == T) {
        ...
        } else if (is.na(else_class) == F) { # 2nd scenario: the rule j has an 'else':
          # part 1:
          # store results in the iteration:
          class_label <- matrix(NA, nrow = dim(oob.sample.balanced)[1],
                                ncol = 2)
          ...
          for (k in 1:dim(oob.sample.balanced)[1]) {
            if (is.na(eval(parse(text = rules.tree.i[[j]]))[out.of.bag[k]]) == F &&
                eval(parse(text = rules.tree.i[[j]]))[out.of.bag[k]] == F && 
        ...
        final_result[footstep,] <- list("Rules" = rules.tree.i[[j]],
                                        "Class Label" = class,
  ...
  final_result_all <- final_rules_table_df[!duplicated(final_rules_table_df),]
...
select.rules <- function(validation.data = data.split$validation.data, rule.table = rules$Rules) {
  ...
  # rule.table <- rule.table[rule.table[,4] != 0,]
  
  # the probability of predicting correctly by random guessing is 0.5*0.5+0.5*0.5 = 0.5
  # hard coding: the 4th column is the performance scores
  # rules.order.temp <- rule.table[rule.table[,4] > 0.5,]
  
  # order the rules by their performance scores:
  # hard coding: the 4th column is the performance scores
  rules.order <- rule.table[order(rule.table[,4], decreasing = T), ]
  ...
  for (i in 1:nrow(decision.rules)) {
    if (i > nrow(decision.rules)) {
      ...
      for (k in 1:dim(data)[1]) {
        ...
        # determine the class label of the kth sample in validation data:
        label_data <- inter.table[inter.table[,1] != "other",]
        predicted_label = table(as.character(label_data[,1]))
        if (is.null(dim(label_data)) == T) {
          # means all classifiers vote for 'other'
          # the final output is the class with the largest number of training samples.
          # train_label = table(as.character(train.data$subtype))
          pred_class[k,1] <- data.split$most.freq
        } else if (is.null(dim(label_data)) == F & length(predicted_label) != 1) {
          # means not all classifiers vote for 'other'
          pred_class[k,1] <- data.split$most.freq
        } else if (is.null(dim(label_data)) == F & length(predicted_label) == 1) {
          # means not all classifiers vote for 'other'
          pred_class[k,1] <- names(predicted_label[predicted_label == max(predicted_label)])
        }
      }
    }
  }
