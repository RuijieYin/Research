      ...
      for (t in 1:nrow(rules.order)) {
        ...
        if (t > nrow(rules.order)) {
          ...
          for (k in 1:dim(validation.data.pair)[1]) {
            ...
            suffice_rule <- inter.table[inter.table[,3] == 1,]
            if (nrow(suffice_rule) != 0) {
              # this means NOT all predictions are 'else class'
              label <- table(as.character(suffice_rule[,1]))
              pred_label <- names(label[label == max(label)])
              if (length(pred_label) != 1) {
                pred_class[k,1] <- as.character(suffice_rule[,1][which.max(suffice_rule[,2])])
              } else {
                pred_class[k,1] <- pred_label
              }
            } else if (nrow(suffice_rule) == 0) {
              # this means NOT all predictions are 'else class'
