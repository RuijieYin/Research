...
source("treeRule_Obtain_Judge.R")
# load split.extract.predict.R
source("split_extract_predict_multiclass_balanced_trees_1v1_ver5.R")


# split the training data (TCGA BRCA 1095 samples: 800 training, 200 validation, 95 test)
data.split <- split.train(data = read.csv(file = "tcga_quantiles_50.csv", header = T, row.names = 1))
...
# rules$Rules
write.csv(rules$Rules, "rules_extracted_1v1_ver2.csv", row.names = F)
...
write.csv(selected.rules$Selected.Rules, "multiclass1_rules_1v1_ver4.csv", row.names = F)


# Predict on the test data (the remaining 95 samples)
pred <- predict.rules(data = data.split$test.data, decision.rules = final.decision.rules.untrim)
