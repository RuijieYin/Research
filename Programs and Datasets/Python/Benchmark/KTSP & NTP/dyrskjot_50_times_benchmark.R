...
filenames <- list.files(pattern="train_dyrskjot_.*csv")
filenames2 <- list.files(pattern="test_dyrskjot_.*csv")
...
for(i in 1:50) {
  assign("train_data_temp", eval(parse(text = paste(text = "train_dyrskjot_", i, sep = ""))))
  train_data = get("train_data_temp")
  # remove the first column as row numbers
  # train_data = train_data[,-1]
  assign("test_data_temp", eval(parse(text = paste(text = "test_dyrskjot_", i, sep = ""))))
  ...
  # 1 vs 1 scheme
  #
  num_class_1 = length(which(train_data$subtype== "T1"))
  num_class_2 = length(which(train_data$subtype== "T2plus"))
  num_class_3 = length(which(train_data$subtype== "TA"))

# KTSP
# 1 vs 1 scheme
#
subtypes1 <- factor(c(rep("T1", num_class_1), rep("T2plus", num_class_2)))
data1 <- as.matrix(cbind(t(train_data[train_data$subtype == "T1", -dim(train_data)[2]]),
                         t(train_data[train_data$subtype == "T2plus", -dim(train_data)[2]])))
...
subtypes2 <- factor(c(rep("T1", num_class_1), rep("TA", num_class_3)))
data2 <- as.matrix(cbind(t(train_data[train_data$subtype == "T1", -dim(train_data)[2]]),
                         t(train_data[train_data$subtype == "TA", -dim(train_data)[2]])))
# training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
# classifier and a function (SWAP.KTSP.Classify) 
# for predicting the label of an unseen sample.
classifier2 <- SWAP.Train.KTSP(data2, subtypes2)

subtypes3 <- factor(c(rep("T2plus", num_class_2), rep("TA", num_class_3)))
data3 <- as.matrix(cbind(t(train_data[train_data$subtype == "T2plus", -dim(train_data)[2]]),
                         t(train_data[train_data$subtype == "TA", -dim(train_data)[2]])))
# training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
# classifier and a function (SWAP.KTSP.Classify) 
# for predicting the label of an unseen sample.
...
write.csv(accuracy, "F:/Dropbox/UM Biostatistics/Research/1st Project/TNBCtype_research/Programs/Benchmarking/exp17 50 replica/Results Benchmark/dyrskjot.results.ktsp.csv", row.names = T)
...
for (j in 1:num_of_features) {
  # T1 T2plus     TA 
  largest <- which.max(c(mean(rank_data_labeled[,j][which(rank_data_labeled$subtype == "T1")]),
                         mean(rank_data_labeled[,j][which(rank_data_labeled$subtype == "T2plus")]),
                         mean(rank_data_labeled[,j][which(rank_data_labeled$subtype == "TA")])))
  template[j,largest] <- 1
  template[j,-largest] <- c(0,0)
}

template_T1 <- template[,1]
template_T2plus <- template[,2]
template_TA <- template[,3]
...
index <- c("T1", "T2plus", "TA")
for (u in 1:ncol(t_test_data_nolabel)) {
  test1 <- cosine(t_test_data_nolabel[,u], template_T1)
  test2 <- cosine(t_test_data_nolabel[,u], template_T2plus)
  test3 <- cosine(t_test_data_nolabel[,u], template_TA)
  inner_results[u,] <- c(test1, test2, test3)
...
accuracy = as.matrix(accuracy, ncol = 1)
write.csv(accuracy, "F:/Dropbox/UM Biostatistics/Research/1st Project/TNBCtype_research/Programs/Benchmarking/exp17 50 replica/Results Benchmark/dyrskjot.results.ntp.csv", row.names = T)
