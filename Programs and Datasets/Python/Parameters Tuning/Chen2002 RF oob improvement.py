# compared to ver1, added some changes to the numbers in learning step
...
train_data = pd.read_csv(
        'F:/Dropbox/UM Biostatistics/Research/1st Project/TNBCtype_research/Programs/Benchmarking/exp1/train1_unconverted.csv')
test_data = pd.read_csv(
        'F:/Dropbox/UM Biostatistics/Research/1st Project/TNBCtype_research/Programs/Benchmarking/exp1/test1_unconverted.csv')
...
n_estimators_numbers = [*range(1,26) , 50,60,70,80,90,100,150,200,250,300,350,400,450,500, 750, 1000]
model = RandomForestClassifier(warm_start=True, oob_score=True)


for i in n_estimators_numbers:
    model.set_params(n_estimators=i)
    model.fit(X_train, y_train)
    oob_score_temp = model.oob_score_
    oob_score.append(oob_score_temp)
...
result_table = {"exp_id": exp_id, "OOB Score": oob_score}
result_table_df = pd.DataFrame(result_table)
result_table_df.to_csv(
    'F:/Dropbox/UM Biostatistics/Research/1st Project/TNBCtype_research/Programs/Benchmarking/exp9 boxplot3/Results/RF_oob_improvement.csv', index=False)
