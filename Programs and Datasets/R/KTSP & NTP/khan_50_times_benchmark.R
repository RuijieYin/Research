...
filenames <- list.files(pattern="train_khan_.*csv")
filenames2 <- list.files(pattern="test_khan_.*csv")
...
for(i in 1:50) {
  assign("train_data_temp", eval(parse(text = paste(text = "train_khan_", i, sep = ""))))
  train_data = get("train_data_temp")
  # remove the first column as row numbers
  # train_data = train_data[,-1]
  assign("test_data_temp", eval(parse(text = paste(text = "test_khan_", i, sep = ""))))
  ...
  # 1 vs 1 scheme
  #
  num_class_1 = length(which(train_data[,2309] == 1))
  num_class_2 = length(which(train_data[,2309] == 2))
  num_class_3 = length(which(train_data[,2309] == 3))
  num_class_4 = length(which(train_data[,2309] == 4))
  
  subtypes1 <- factor(c(rep(1, num_class_1), rep(2, num_class_2)))
  data1 <- as.matrix(cbind(t(train_data[train_data[,2309] == 1, -dim(train_data)[2]]),
                           t(train_data[train_data[,2309] == 2, -dim(train_data)[2]])))
  ...
  subtypes2 <- factor(c(rep(1, num_class_1), rep(3, num_class_3)))
  data2 <- as.matrix(cbind(t(train_data[train_data[,2309] == 1, -dim(train_data)[2]]),
                           t(train_data[train_data[,2309] == 3, -dim(train_data)[2]])))
  # training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
  # classifier and a function (SWAP.KTSP.Classify) 
  # for predicting the label of an unseen sample.
  classifier2 <- SWAP.Train.KTSP(data2, subtypes2)
  
  subtypes3 <- factor(c(rep(1, num_class_1), rep(4, num_class_4)))
  data3 <- as.matrix(cbind(t(train_data[train_data[,2309] == 1, -dim(train_data)[2]]),
                           t(train_data[train_data[,2309] == 4, -dim(train_data)[2]])))
  # training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
  # classifier and a function (SWAP.KTSP.Classify) 
  # for predicting the label of an unseen sample.
  classifier3 <- SWAP.Train.KTSP(data3, subtypes3)
  
  subtypes4 <- factor(c(rep(2, num_class_2), rep(3, num_class_3)))
  data4 <- as.matrix(cbind(t(train_data[train_data[,2309] == 2, -dim(train_data)[2]]),
                           t(train_data[train_data[,2309] == 3, -dim(train_data)[2]])))
  # training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
  # classifier and a function (SWAP.KTSP.Classify) 
  # for predicting the label of an unseen sample.
  ...
  subtypes5 <- factor(c(rep(2, num_class_2), rep(4, num_class_4)))
  data5 <- as.matrix(cbind(t(train_data[train_data[,2309] == 2, -dim(train_data)[2]]),
                           t(train_data[train_data[,2309] == 4, -dim(train_data)[2]])))
  # training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
  # classifier and a function (SWAP.KTSP.Classify) 
  # for predicting the label of an unseen sample.
  ...
  subtypes6 <- factor(c(rep(3, num_class_3), rep(4, num_class_4)))
  data6 <- as.matrix(cbind(t(train_data[train_data[,2309] == 3, -dim(train_data)[2]]),
                           t(train_data[train_data[,2309] == 4, -dim(train_data)[2]])))
  # training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
  # classifier and a function (SWAP.KTSP.Classify) 
  # for predicting the label of an unseen sample.
  ...
  compare_labels <- rbind(matrix(test_data[,2309], nrow=1), 
                          pred_result_t)
...
write.csv(accuracy, "F:/Dropbox/UM Biostatistics/Research/1st Project/TNBCtype_research/Programs/Benchmarking/exp17 50 replica/Results Benchmark/khan.results.ktsp.csv", row.names = T)
...
for (j in 1:num_of_features) {
  # 1,2,3,4
  largest <- which.max(c(mean(rank_data_labeled[,j][which(rank_data_labeled$subtype == 1)]),
                         mean(rank_data_labeled[,j][which(rank_data_labeled$subtype == 2)]),
                         mean(rank_data_labeled[,j][which(rank_data_labeled$subtype == 3)]),
                         mean(rank_data_labeled[,j][which(rank_data_labeled$subtype == 4)])))
  template[j,largest] <- 1
  template[j,-largest] <- c(0,0,0)
}
...
for (t in 1:ncol(t_test_data_nolabel)) {
  test1 <- cosine(t_test_data_nolabel[,t], template_1)
  test2 <- cosine(t_test_data_nolabel[,t], template_2)
  test3 <- cosine(t_test_data_nolabel[,t], template_3)
  test4 <- cosine(t_test_data_nolabel[,t], template_4)
  inner_results[t,] <- c(test1, test2, test3, test4)
  largest_corr <- which.max(c(test1, test2, test3, test4))
...
accuracy = as.matrix(accuracy, ncol = 1)
write.csv(accuracy, "F:/Dropbox/UM Biostatistics/Research/1st Project/TNBCtype_research/Programs/Benchmarking/exp17 50 replica/Results Benchmark/khan.results.ntp.csv", row.names = T)
