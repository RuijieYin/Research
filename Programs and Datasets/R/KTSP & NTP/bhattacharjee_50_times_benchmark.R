...
filenames <- list.files(pattern="train_bhattacharjee_.*csv")
filenames2 <- list.files(pattern="test_bhattacharjee_.*csv")
...
for(i in 1:50) {
  assign("train_data_temp", eval(parse(text = paste(text = "train_bhattacharjee_", i, sep = ""))))
  train_data = get("train_data_temp")
  # remove the first column as row numbers
  # train_data = train_data[,-1]
  assign("test_data_temp", eval(parse(text = paste(text = "test_bhattacharjee_", i, sep = ""))))
  ...
  # 1 vs 1 scheme
  #
  num_class_1 = length(which(train_data$subtype== "AD"))
  num_class_2 = length(which(train_data$subtype== "COID"))
  num_class_3 = length(which(train_data$subtype== "NL"))
  num_class_4 = length(which(train_data$subtype== "SCLC"))
  num_class_5 = length(which(train_data$subtype== "SQ"))
# KTSP
# 1 vs 1 scheme
#
subtypes1 <- factor(c(rep("AD", num_class_1), rep("COID", num_class_2)))
data1 <- as.matrix(cbind(t(train_data[train_data$subtype == "AD", -dim(train_data)[2]]),
                         t(train_data[train_data$subtype == "COID", -dim(train_data)[2]])))
...
subtypes2 <- factor(c(rep("AD", num_class_1), rep("NL", num_class_3)))
data2 <- as.matrix(cbind(t(train_data[train_data$subtype == "AD", -dim(train_data)[2]]),
                         t(train_data[train_data$subtype == "NL", -dim(train_data)[2]])))
# training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
# classifier and a function (SWAP.KTSP.Classify) 
# for predicting the label of an unseen sample.
classifier2 <- SWAP.Train.KTSP(data2, subtypes2)

subtypes3 <- factor(c(rep("AD", num_class_1), rep("SCLC", num_class_4)))
data3 <- as.matrix(cbind(t(train_data[train_data$subtype == "AD", -dim(train_data)[2]]),
                         t(train_data[train_data$subtype == "SCLC", -dim(train_data)[2]])))
# training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
# classifier and a function (SWAP.KTSP.Classify) 
# for predicting the label of an unseen sample.
...
subtypes4 <- factor(c(rep("AD", num_class_1), rep("SQ", num_class_5)))
data4 <- as.matrix(cbind(t(train_data[train_data$subtype == "AD", -dim(train_data)[2]]),
                         t(train_data[train_data$subtype == "SQ", -dim(train_data)[2]])))
# training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
# classifier and a function (SWAP.KTSP.Classify) 
# for predicting the label of an unseen sample.
...
subtypes5 <- factor(c(rep("COID", num_class_2), rep("NL", num_class_3)))
data5 <- as.matrix(cbind(t(train_data[train_data$subtype == "COID", -dim(train_data)[2]]),
                         t(train_data[train_data$subtype == "NL", -dim(train_data)[2]])))
# training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
# classifier and a function (SWAP.KTSP.Classify) 
# for predicting the label of an unseen sample.
...
subtypes6 <- factor(c(rep("COID", num_class_2), rep("SCLC", num_class_4)))
data6 <- as.matrix(cbind(t(train_data[train_data$subtype == "COID", -dim(train_data)[2]]),
                         t(train_data[train_data$subtype == "SCLC", -dim(train_data)[2]])))
# training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
# classifier and a function (SWAP.KTSP.Classify) 
# for predicting the label of an unseen sample.
...
subtypes7 <- factor(c(rep("COID", num_class_2), rep("SQ", num_class_5)))
data7 <- as.matrix(cbind(t(train_data[train_data$subtype == "COID", -dim(train_data)[2]]),
                         t(train_data[train_data$subtype == "SQ", -dim(train_data)[2]])))
# training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
# classifier and a function (SWAP.KTSP.Classify) 
# for predicting the label of an unseen sample.
...
subtypes8 <- factor(c(rep("NL", num_class_3), rep("SCLC", num_class_4)))
data8 <- as.matrix(cbind(t(train_data[train_data$subtype == "NL", -dim(train_data)[2]]),
                         t(train_data[train_data$subtype == "SCLC", -dim(train_data)[2]])))
# training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
# classifier and a function (SWAP.KTSP.Classify) 
# for predicting the label of an unseen sample.
...
subtypes9 <- factor(c(rep("NL", num_class_3), rep("SQ", num_class_5)))
data9 <- as.matrix(cbind(t(train_data[train_data$subtype == "NL", -dim(train_data)[2]]),
                         t(train_data[train_data$subtype == "SQ", -dim(train_data)[2]])))
# training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
# classifier and a function (SWAP.KTSP.Classify) 
# for predicting the label of an unseen sample.
...
subtypes10 <- factor(c(rep("SCLC", num_class_4), rep("SQ", num_class_5)))
data10 <- as.matrix(cbind(t(train_data[train_data$subtype == "SCLC", -dim(train_data)[2]]),
                         t(train_data[train_data$subtype == "SQ", -dim(train_data)[2]])))
# training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
# classifier and a function (SWAP.KTSP.Classify) 
# for predicting the label of an unseen sample.
...
write.csv(accuracy, "F:/Dropbox/UM Biostatistics/Research/1st Project/TNBCtype_research/Programs/Benchmarking/exp17 50 replica/Results Benchmark/bhattacharjee.results.ktsp.csv", row.names = T)
...
for (u in 1:num_of_features) {
  # AD COID   NL SCLC   SQ 
  largest <- which.max(c(mean(rank_data_labeled[,u][which(rank_data_labeled$subtype == "AD")]),
                         mean(rank_data_labeled[,u][which(rank_data_labeled$subtype == "COID")]),
                         mean(rank_data_labeled[,u][which(rank_data_labeled$subtype == "NL")]),
                         mean(rank_data_labeled[,u][which(rank_data_labeled$subtype == "SCLC")]),
                         mean(rank_data_labeled[,u][which(rank_data_labeled$subtype == "SQ")]))
                       )
  template[u,largest] <- 1
  template[u,-largest] <- c(0,0,0,0)
}

template_AD <- template[,1]
template_COID <- template[,2]
template_NL <- template[,3]
template_SCLC <- template[,4]
template_SQ <- template[,5]
...
index <- c("AD", "COID", "NL", "SCLC", "SQ")
for (t in 1:ncol(t_test_data_nolabel)) {
  test1 <- cosine(t_test_data_nolabel[,t], template_AD)
  test2 <- cosine(t_test_data_nolabel[,t], template_COID)
  test3 <- cosine(t_test_data_nolabel[,t], template_NL)
  test4 <- cosine(t_test_data_nolabel[,t], template_SCLC)
  test5 <- cosine(t_test_data_nolabel[,t], template_SQ)
  inner_results[t,] <- c(test1, test2, test3, test4, test5)
...
accuracy = as.matrix(accuracy, ncol = 1)
write.csv(accuracy, "F:/Dropbox/UM Biostatistics/Research/1st Project/TNBCtype_research/Programs/Benchmarking/exp17 50 replica/Results Benchmark/bhattacharjee.results.ntp.csv", row.names = T)
