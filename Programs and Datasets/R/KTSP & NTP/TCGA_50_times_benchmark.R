...
filenames <- list.files(pattern="train_TCGA_.*csv")
filenames2 <- list.files(pattern="test_TCGA_.*csv")
...
for(i in 1:50) {
  assign("train_data_temp", eval(parse(text = paste(text = "train_TCGA_", i, sep = ""))))
  train_data = get("train_data_temp")
  # remove the first column as row numbers
  # train_data = train_data[,-1]
  assign("test_data_temp", eval(parse(text = paste(text = "test_TCGA_", i, sep = ""))))
  ...
  # 1 vs 1 scheme
  #
  num_class_1 = length(which(train_data$subtype== "BRCA.Basal"))
  num_class_2 = length(which(train_data$subtype== "BRCA.LumA"))
  num_class_3 = length(which(train_data$subtype== "BRCA.LumB"))
  num_class_4 = length(which(train_data$subtype== "BRCA.Normal"))
  num_class_5 = length(which(train_data$subtype== "BRCA.Her2"))

  
  # KTSP
  # 1 vs 1 scheme
  #
  subtypes1 <- factor(c(rep("BRCA.Basal", num_class_1), rep("BRCA.LumA", num_class_2)))
  data1 <- as.matrix(cbind(t(train_data[train_data$subtype == "BRCA.Basal", -dim(train_data)[2]]),
                           t(train_data[train_data$subtype == "BRCA.LumA", -dim(train_data)[2]])))
  ...
  subtypes2 <- factor(c(rep("BRCA.Basal", num_class_1), rep("BRCA.LumB", num_class_3)))
  data2 <- as.matrix(cbind(t(train_data[train_data$subtype == "BRCA.Basal", -dim(train_data)[2]]),
                           t(train_data[train_data$subtype == "BRCA.LumB", -dim(train_data)[2]])))
  # training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
  # classifier and a function (SWAP.KTSP.Classify) 
  # for predicting the label of an unseen sample.
  classifier2 <- SWAP.Train.KTSP(data2, subtypes2)
  
  subtypes3 <- factor(c(rep("BRCA.Basal", num_class_1), rep("BRCA.Normal", num_class_4)))
  data3 <- as.matrix(cbind(t(train_data[train_data$subtype == "BRCA.Basal", -dim(train_data)[2]]),
                           t(train_data[train_data$subtype == "BRCA.Normal", -dim(train_data)[2]])))
  # training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
  # classifier and a function (SWAP.KTSP.Classify) 
  # for predicting the label of an unseen sample.
  ...
  subtypes4 <- factor(c(rep("BRCA.Basal", num_class_1), rep("BRCA.Her2", num_class_5)))
  data4 <- as.matrix(cbind(t(train_data[train_data$subtype == "BRCA.Basal", -dim(train_data)[2]]),
                           t(train_data[train_data$subtype == "BRCA.Her2", -dim(train_data)[2]])))
  # training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
  # classifier and a function (SWAP.KTSP.Classify) 
  # for predicting the label of an unseen sample.
  ...
  subtypes5 <- factor(c(rep("BRCA.LumA", num_class_2), rep("BRCA.LumB", num_class_3)))
  data5 <- as.matrix(cbind(t(train_data[train_data$subtype == "BRCA.LumA", -dim(train_data)[2]]),
                           t(train_data[train_data$subtype == "BRCA.LumB", -dim(train_data)[2]])))
  # training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
  # classifier and a function (SWAP.KTSP.Classify) 
  # for predicting the label of an unseen sample.
  ...
  subtypes6 <- factor(c(rep("BRCA.LumA", num_class_2), rep("BRCA.Normal", num_class_4)))
  data6 <- as.matrix(cbind(t(train_data[train_data$subtype == "BRCA.LumA", -dim(train_data)[2]]),
                           t(train_data[train_data$subtype == "BRCA.Normal", -dim(train_data)[2]])))
  # training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
  # classifier and a function (SWAP.KTSP.Classify) 
  # for predicting the label of an unseen sample.
  ...
  subtypes7 <- factor(c(rep("BRCA.LumA", num_class_2), rep("BRCA.Her2", num_class_5)))
  data7 <- as.matrix(cbind(t(train_data[train_data$subtype == "BRCA.LumA", -dim(train_data)[2]]),
                           t(train_data[train_data$subtype == "BRCA.Her2", -dim(train_data)[2]])))
  # training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
  # classifier and a function (SWAP.KTSP.Classify) 
  # for predicting the label of an unseen sample.
  ...
  subtypes8 <- factor(c(rep("BRCA.LumB", num_class_3), rep("BRCA.Normal", num_class_4)))
  data8 <- as.matrix(cbind(t(train_data[train_data$subtype == "BRCA.LumB", -dim(train_data)[2]]),
                           t(train_data[train_data$subtype == "BRCA.Normal", -dim(train_data)[2]])))
  # training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
  # classifier and a function (SWAP.KTSP.Classify) 
  # for predicting the label of an unseen sample.
  ...
  subtypes9 <- factor(c(rep("BRCA.LumB", num_class_3), rep("BRCA.Her2", num_class_5)))
  data9 <- as.matrix(cbind(t(train_data[train_data$subtype == "BRCA.LumB", -dim(train_data)[2]]),
                           t(train_data[train_data$subtype == "BRCA.Her2", -dim(train_data)[2]])))
  # training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
  # classifier and a function (SWAP.KTSP.Classify) 
  # for predicting the label of an unseen sample.
  ...
  subtypes10 <- factor(c(rep("BRCA.Normal", num_class_4), rep("BRCA.Her2", num_class_5)))
  data10 <- as.matrix(cbind(t(train_data[train_data$subtype == "BRCA.Normal", -dim(train_data)[2]]),
                            t(train_data[train_data$subtype == "BRCA.Her2", -dim(train_data)[2]])))
  # training function (SWAP.KTSP.Train:deprecated, use SWAP.Train.KTSP instead) for the 
  # classifier and a function (SWAP.KTSP.Classify) 
  # for predicting the label of an unseen sample.
...
write.csv(accuracy, "F:/Dropbox/UM Biostatistics/Research/1st Project/TNBCtype_research/Programs/Benchmarking/exp17 50 replica/Results Benchmark/TCGA.results.ktsp.csv", row.names = T)
...
for(i in 1:50){
  ...
  for (j in 1:num_of_features) {
    ...
    # num_class_4 = length(which(train_data$subtype== "BRCA.Normal"))
    # num_class_5 = length(which(train_data$subtype== "BRCA.Her2"))
    largest <- which.max(c(mean(rank_data_labeled[,j][which(rank_data_labeled$subtype == "BRCA.Basal")]),
                           mean(rank_data_labeled[,j][which(rank_data_labeled$subtype == "BRCA.LumA")]),
                           mean(rank_data_labeled[,j][which(rank_data_labeled$subtype == "BRCA.LumB")]),
                           mean(rank_data_labeled[,j][which(rank_data_labeled$subtype == "BRCA.Normal")]),
                           mean(rank_data_labeled[,j][which(rank_data_labeled$subtype == "BRCA.Her2")])
    )
    )
    template[j,largest] <- 1
    template[j,-largest] <- c(0,0,0,0)
  }
  
  template_Basal <- template[,1]
  template_LumA <- template[,2]
  template_LumB <- template[,3]
  template_Normal <- template[,4]
  template_Her2 <- template[,5]
  ...
  index <- c("BRCA.Basal", "BRCA.LumA", "BRCA.LumB", "BRCA.Normal", "BRCA.Her2")
  for (u in 1:ncol(t_test_data_nolabel)) {
    test1 <- cosine(t_test_data_nolabel[,u], template_Basal)
    test2 <- cosine(t_test_data_nolabel[,u], template_LumA)
    test3 <- cosine(t_test_data_nolabel[,u], template_LumB)
    test4 <- cosine(t_test_data_nolabel[,u], template_Normal)
    test5 <- cosine(t_test_data_nolabel[,u], template_Her2)
    inner_results[u,] <- c(test1, test2, test3, test4, test5)
    largest_corr <- which.max(c(test1, test2, test3, test4, test5))
...
accuracy = as.matrix(accuracy, ncol = 1)
write.csv(accuracy, "F:/Dropbox/UM Biostatistics/Research/1st Project/TNBCtype_research/Programs/Benchmarking/exp17 50 replica/Results Benchmark/TCGA.results.ntp.csv", row.names = T)
